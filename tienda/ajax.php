...
if (!empty($_POST)) {
    ...
    if ($_POST['action'] == 'infoProducto') {
        ...
        $query = mysqli_query($conection, "SELECT cod_producto, cod_barra, descripcion, existencia, precio FROM producto WHERE cod_producto = $producto_id AND estatus = 1");
    ...
    if ($_POST['action'] == 'addProductoDetalle') {
        if (empty($_POST['producto']) || empty($_POST['cantidad'])) {
            ...
            $result_token = mysqli_num_rows($query_detalle_temp);
            ...
            if ($result_token > 0) {
                ...
                while ($data = mysqli_fetch_assoc($query_detalle_temp)) {
                    ...
                    $detalleTabla .= '<tr>
                                            ...
                                            <td><a class="link_delete" href="#" onclick="event.preventDefault(); del_product_detalle(' . $data['correlativo'] . ');"><i class="far fa-trash-alt"></i></a></td>
                                        </tr>';
                }
    ...
    if ($_POST['action'] == 'serchForDetalle') {
        if (empty($_POST['user'])) {
            ...
            $query_token = mysqli_query($conection, "SELECT tmp.correlativo, 
                                                            tmp.token_user, 
                                                            tmp.cantidad, 
                                                            tmp.precio_venta, 
                                                            p.cod_producto, 
            ...
            $result_token = mysqli_num_rows($query_token);
            ...
            if ($result_token > 0) {
                ...
                while ($data = mysqli_fetch_assoc($query_token)) {
                    $precioTotal = round($data['cantidad'] * $data['precio_venta'], 2);
                    $sub_total = round($sub_total + $precioTotal, 2);
                    $total = round($total + $precioTotal, 2);
    ...
    if ($_POST['action'] == 'infoFactura') {
        if (!empty($_POST['nofactura'])) {
            ...
            $query = mysqli_query($conection, "SELECT * FROM factura WHERE no_factura = '$nofactura' AND estatus = 1");
            mysqli_close($conection);
            $result = mysqli_num_rows($query);
        ...
        if (!empty($_POST['passActual']) && !empty($_POST['passNuevo'])) {
            ...
            if ($result > 0) {
                ...
                if ($query_update) {
                    ...
                    $code = '2';
                    $msg = "No fue posible modificar su contraseña.";
                }
            } else {
                $code = '1';
    ...
    if ($_POST['action'] == 'updateDataEmpresa') {
        if ((empty($_POST['txtRuc']) || empty($_POST['txtNombre']) || empty($_POST['txtTelEmpresa'])) || empty($_POST['txtEmailEmpresa']) || empty($_POST['txtDirEmpresa']) || empty($_POST['txtIva'])) {
            $code = '1';
            ...
            $queryUpd = mysqli_query($conection, "UPDATE configuracion SET ruc = '$intNit', nombre = '$strNombre', razon_social = '$strRSocial', telefono = '$intTel', email = '$strEmail', direccion = '$strDir', iva = $strIva WHERE id = 1");
            mysqli_close($conection);
